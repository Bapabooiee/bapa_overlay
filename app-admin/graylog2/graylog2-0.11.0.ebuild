...
SRC_URI="http://download.graylog2.org/graylog2-server/${MY_P}.tar.gz"
...
src_install() {
	...
	newinitd "${FILESDIR}/${PN}-0.11.0.initd" ${PN} || die "newinitd failed"
	newconfd "${FILESDIR}/${PN}-0.11.0.confd" ${PN} || die "newconfd failed"
}
