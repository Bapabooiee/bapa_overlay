...
EJABBERD_MODULES="mod_irc mod_muc mod_proxy65 mod_pubsub mod_statsdx \
	mod_cron mod_s2s_log xmlrpc"
IUSE="captcha debug ldap odbc pam +web zlib ${EJABBERD_MODULES} ejab1381"
...
DEPEND=">=net-im/jabber-base-0.01
	...
	xmlrpc? ( >=dev-libs/xmlrpc-erlang-ipr2-1.13 )
	zlib? ( sys-libs/zlib )"
...
src_prepare() {
	...
	# adding community modules source files
	if use xmlrpc ; then
		epatch "${FILESDIR}/ejabberd_xmlrpc_svn1102.patch"
	fi
        # adding patch from https://support.process-one.net/browse/EJAB-1381
        if use ejab1381 ; then
                epatch "${FILESDIR}/ejabberd_ejab1381-2111.patch"
        fi

	# older patches require change to work dir, not /src
	cd ${WORKDIR}/${P}
	if use mod_cron ; then
		epatch "${FILESDIR}/mod_cron_svn1034.patch"
	fi
	if use mod_s2s_log ; then
		epatch "${FILESDIR}/mod_s2s_log_svn701.patch"
	fi
	cd ${S}
...
pkg_postinst() {
	...
	# community modules information
        if use use mod_cron || use mod_s2s_log || use xmlrpc ; then
		elog "One or more community modules installed, to activate and configure "
		elog "them read their documentation on http://www.ejabberd.im/contributions"
        fi

	# ejab-1381 information
	if use ejab138 ; then
		elog "Patch from #EJAB-1381 for remote roster management activated,"
		elog "read the documentation at http://spectrum.im/projects/spectrum/wiki/Remote_roster"
	fi
}
